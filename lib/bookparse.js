...
async function epubLines(file,prog){
  ...
  const put=(src,text)=>{
    if(!src||!text) return;
    const key=decodeURIComponent(String(src).split('#')[0]);
    if(key&&!label[key]) label[key]=text.replace(/\s+/g,' ').trim();
  };
  if(tocPath){
