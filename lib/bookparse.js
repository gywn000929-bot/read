...
function docsToBook(docs,meta){
  ...
  /* 한 파일 안에서도 목차가 가리킨 자리마다 장이 갈린다 (l.brk 가 그 표시).
     ...
     10개가 한 장으로 뭉친다. */
  /* 목차 이름과 본문 첫 줄이 '같은 것인가' 를 견주는 잣대. 눈에는 같아 보여도
     글자로는 다른 경우가 많아, 견줄 때만 한 꼴로 눌러 놓는다 (보여 줄 때는 원본 그대로).
       · NFKC — 로마 숫자 글자 Ⅱ→II, 전각 Ｖ→V, 전각 마침표 ．→.
       · 안 보이는 글자 — 안 깨지는 빈칸(nbsp)·너비 없는 빈칸(zwsp)·BOM
       · 문장 부호와 대소문자
     이걸 안 하면 목차가 'II' 인데 본문이 'Ⅱ' 인 책에서 제목이 두 번 나온다. */
  const bare=x=>String(x||'').normalize('NFKC')
    .replace(/[\s\u00a0\u200b-\u200d\ufeff]+/g,'')
    .replace(/[.,·・‧:;'"“”‘’「」『』()\[\]{}!?\-–—_*]/g,'')
    .toLowerCase();
  const pieces=[];
  ...
  return plated(chapters,meta,bare);
}
/* 표제 쪽 걷어내기.
   한국 전자책의 목차에는 '제1부 바다와 밤' 처럼 이름만 있고 글은 없는 쪽이
   장으로 끼어 있다. 열어 봐야 제목 한 줄뿐인데 목차에서 자리를 차지하고,
   '몇 장 읽었나' 도 그만큼 부풀린다. 웃는 남자는 69장 중 10개가 그런 쪽이었다.

   무엇이 표제 쪽인가 — 문단이 셋 이하이고, 문단마다 둘 중 하나인 쪽:
     · 제 제목의 되풀이           (「제1권 …」 안에 「제1권 …」)
     · 한글이 없는 짧은 로마자 줄  (「제1부 바다와 밤」 안에 "La mer et la nuit")
   한글로 된 제 글이 한 줄이라도 있으면 표제 쪽이 아니다. 이 한 줄이 짧은 시와
   제사(題詞)를 지킨다 — 카뮈 여름의 「하지만 너는 맑게 갠 날을 …」 은 본문이
   '—횔덜린' 뿐이지만 제목의 되풀이가 아니므로 그대로 장으로 남는다.

   찾은 표제 쪽은 버리지 않고 옆단에서 뒤따르는 장들을 묶는 이름으로 올린다 —
   '제1부·제2부' 라는 책의 짜임은 읽는 사람이 알아야 할 것이지, 눌러 봐야
   빈 쪽이 나오는 장이 아니다. 앞머리(속표지·원제)는 묶을 것이 뒤에 없으니 버린다. */
const HANGUL=/[가-힣ㄱ-ㅎㅏ-ㅣ]/;
function plated(chapters,meta,bare){
  const isPlate=c=>{
    if(!c.paras.length||c.paras.length>3) return false;
    const t=bare(c.title);
    let left=0;
    for(const x of c.paras){
      const n=bare(x);
      if(!n) continue;
      if(t&&t.includes(n)) continue;        // 제목의 되풀이(<br/> 로 갈린 토막 포함)
      if(HANGUL.test(n)) return false;      // 제 글이 있다
      left+=n.length;                       // 원어 제목 같은 로마자 한 줄
    }
    return left<=40;
  };
  /* 속표지는 제 제목의 되풀이가 아니라 '원제 · 지은이 · 옮긴이' 를 담고 있어서
     위 잣대에 걸리지 않는다. 맨 앞이면서 제목이 책 이름 그대로이고, 짧은 몇 줄이
     전부일 때만 속표지로 본다 — 책 이름을 단 첫 글(표제작)을 지우지 않으려고
     글자 수로 한 번 더 잠근다. */
  const key=x=>String(x||'').replace(/[^0-9A-Za-z가-힣]/g,'');
  const isCover=(c,i)=>i===0 && !!key(meta.title) && key(c.title)===key(meta.title)
    && c.paras.length<=5 && c.paras.join('').length<=120;
  const plate=chapters.map((c,i)=>isPlate(c)||isCover(c,i));
  const firstReal=plate.indexOf(false);
  if(firstReal<0||!plate.some(Boolean))
    return pack(chapters,meta,{});          // 표제 쪽이 없거나 온통 표제 쪽이면 그대로
  const out=[], parts={}; let part=1, waiting=[];
  chapters.forEach((c,i)=>{
    if(plate[i]){
      if(i<firstReal) return;               // 앞머리 — 묶을 장이 뒤에 없다
      waiting.push(c.title); return;
    }
    if(waiting.length){ part++; parts[part]=waiting.join(' · '); waiting=[]; }
    out.push({...c,part});
  });
  out.forEach((c,i)=>{ c.n=i+1; });
  /* 첫 묶음(앞에 표제 쪽이 없던 장들)에는 책이 붙여 준 이름이 없다.
     parts 에 넣지 않고 비워 둔다 — 옆단은 이름 없는 묶음의 머리글을 건너뛴다. */
  return pack(out,meta,parts);
}
function pack(chapters,meta,parts){
...
/* 줄표(—·–)도 턴다. 레미제라블은 장 머리가 "CHAPTER I—M. MYRIEL" 이라
   그냥 두면 제목이 "Chapter 1 · —M. MYRIEL" 이 된다. */
...
function linesToBook(inp,meta){
  ...
  /* 60자가 넘어 제목으로 안 보이던 장 머리를 받는다. 노트르담 드 파리는 장 이름이
     길어 59장 중 9개가 60자를 넘겼고, 그 9개가 앞 장에 통째로 붙어 52장이 됐다.
     'CHAPTER <번호>' 로 시작하면서 온통 대문자인 줄만 받으므로 본문과 헷갈리지 않는다. */
  const LONGHEAD=/^(chapter|book|part)\s+(\d{1,3}|[ivxlcdm]{1,7})\b[.\s—–-]/i;
  const bigTxt=s=> s.length<60 || (s.length<=110 && LONGHEAD.test(s) && s===s.toUpperCase());
  const big=l=> inp.hasFont ? (bodyH>0 && l.h>=bodyH*1.15 && l.s.length<=44) : bigTxt(l.s);
  ...
  /* 장 이름이 통째로 대문자로 박힌 판이 있다 — 레미제라블·노트르담이 그렇다.
     서재의 다른 책들은 대소문자가 섞여 있어서, 목차만 소리치듯 보인다.
     라틴 대문자만 있고 소문자가 하나도 없을 때만 손댄다 — 한글은 그대로 둔다. */
  const subCase=t=>(/[A-Z]/.test(t)&&!/[a-z]/.test(t))? titleCase(t) : t;
  const norm=x=>String(x||'').toUpperCase().replace(/[^A-Z0-9]/g,'');
  const titleKey=norm(meta.title);
  const sameAsTitle=t=>!!titleKey && norm(t)===titleKey;
  function scan(partsAsChapters){
  const chapters=[]; const partName={}; let part=1, cur=null, prev=null, numbered=0;
  /* "CHAPTER XIII." 한 줄, 빈 줄, 그리고 "FACE TO FACE WITH NIGHT." — 장 이름을
     다음 줄에 따로 두는 판이 구텐베르크에 흔하다. 그냥 두면 제목은 번호뿐이고
     (한 책에 'Chapter 1' 이 여덟 번 나온다) 정작 이름은 본문 첫 문단이 된다. */
  let wantSub=false;
  const open=(title)=>{ cur={n:chapters.length+1,title,part,paras:[]}; chapters.push(cur); prev=null;
    wantSub=/^(chapter|book)\s/i.test(title)&&!/ · /.test(title);
  ...
  lines.forEach(l=>{
    ...
    if(big(l)){
      ...
      if((m=s.match(RX_CHAP))){ const n=rnum(m[1])||wnum(m[1]);
        const sub=subCase(clean(m[2]));
        open('Chapter '+(n||m[1])+(sub?' · '+sub:'')); return; }
      /* 책 이름이 숫자말이면 그것이 장 번호로 읽힌다 — 표제지의 'NINETY-THREE' 가
         93장이 되어 목차 맨 앞에 앉았다. 책 이름과 같은 줄은 장이 아니다. */
      const wn=wnum(s);
      if(wn && !sameAsTitle(s)){ open('Chapter '+wn); return; }      // "ONE", "TWENTY-TWO"
      if(RX_NUM.test(s)){ open('Chapter '+s); return; }              // "1", "2"
      if((m=s.match(RX_NUMT)) && +m[1]===numbered+1){
        open('Chapter '+(+m[1])+' · '+subCase(clean(m[2]))); return; }
      if((m=s.match(RX_ROMANT)) && rnum(m[1])===numbered+1){
        open('Chapter '+rnum(m[1])+' · '+subCase(clean(m[2]))); return; }
      ...
      if((m=s.match(RX_SECT))){
        ...
        if(n){ const sub=subCase(clean(m[3]));
          open(titleCase(m[1])+' '+n+(sub?' · '+sub:'')); return; } }
      if(inp.hasFont){ open(titleCase(s)); return; }                 // 그 밖의 큰 글씨 = 제목
    }
    if(!cur) open('Opening');
    if(wantSub){
      wantSub=false;                                  // 바로 다음 줄에만 준다
      /* 온통 대문자로 된 짧은 두세 마디 — 그것이 그 장의 이름이다.
         쉼표로 끝나는 것은 받지 않는다 ("DEAR SIR," 같은 편지 첫머리) */
      const ws=s.split(/\s+/).filter(Boolean).length;
      if(!cur.paras.length && s.length<=64 && ws>=2 && ws<=12
         && /[A-Za-zÀ-ÖØ-Þ]/.test(s) && s===s.toUpperCase() && !/[,;:]$/.test(s)){
        const sub=clean(titleCase(s));
        if(sub){ cur.title+=' · '+sub; prev=null; return; }
      }
    }
