...
function looksLineBroken(t){
  ...
  return med<=45 && ends<0.6;
}
function mergeLines(t){
  const out=[];
  for(const line of t){
    if(out.length && !SENT_END.test(out[out.length-1])) out[out.length-1]+=' '+line;
    else out.push(line);
  }
  return out;
}
...
function xhtmlLines(html,out,opts){
  ...
  const flush=()=>{
    ...
    const t=looksLineBroken(seg)? mergeLines(seg) : seg;
...
async function epubLines(file,prog){
  ...
  const titleish=t=>{ t=String(t||'').trim();
